    ...
    ws.on('message', data => {
      ...
      if(jsonData.station.id == 1){
        (async function(){
          ...
          client.user.setActivity(`${client.apiData[1].now_playing.song.artist}  - ${client.apiData[1].now_playing.song.title} | ${guilds.reduce((prev, guildCount) => prev + guildCount, 0)} servers | shard ${client.shard.ids[0] + 1} of ${client.shard.count} | c!help | ChickenFM.com`, { type: 'LISTENING' })  
        })()
      }
    });
